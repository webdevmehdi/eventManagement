...
import {
  requestImageLivreModif,
  requestImage,
} from "../../../../Middleware/Library-Middleware-thunk/Category-Middleware-thunk";
import { Document, Page } from "react-pdf";
import { deleteLivre } from "../../../../Middleware/Library-Middleware-thunk/Category-Middleware-thunk";
import axios from "axios";
import "./CardLivre.css";

const CardLivre = (props) => {
  const [livreModifSelectione, setLivreModifSelectione] = useState("");
  const [selected, setSelected] = useState(false);
  const [image, setImage] = useState(null);
  const [file, setFile] = useState();
  //const [deleteLivreHandler, setDeleteLivreHandler] = useState();
  const deleteLivreHandler = () => {
    deleteLivre(props.fileId).then((response) => {
      setSelected(false);
    });
  };
  // get pdf file
  useEffect(() => {
    axios
      .get(`http://localhost:8081/getImage/${props.fileId}`, {
  ...
  const livreImageHandler = () => {
    let fd = new FormData();
    fd.append("image", livreModifSelectione, livreModifSelectione.name);
    requestImageLivreModif(fd);
    setSelected(true);
  };

  //on change event
  const imageSlectHandler = (e) => {
    console.log(e.target.files[0]);

    setLivreModifSelectione(e.target.files[0]);
  ...
  //download file from data base
  const imageDownloadHandler = () => {
    requestImage(); //.then((response) => setImage(response));
  };
  return (
    <>
      <div className="cardLivreContainer">
        <Document
          //loading={"please wait"}
      ...
      <button style={{ margin: "2px" }} class="" class="btn btn-primary">
        ...
        <div className="livreButtons">
          <button className="btn btn-primary" onClick={deleteLivreHandler}>
            Supprimer Livre
          </button>
          <button className="btn btn-primary" onClick={handleBookRead}>
            Lire Livre
          </button>
        </div>
      </div>

      {/* image livre sinon image categorie */}
      {/* Nom du livre */}
    </>
  );
};
